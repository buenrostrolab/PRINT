...
# Function used to compute observed bias from a count tensor 
getObsBias <- function(
    counts, # Count tensor generated using getCountTensor()
    regionInd, # Index of the region
    regionSize # Width of the region
    ){
  
  # Get Tn5 insertion in the current region
  regionCounts <- counts[[regionInd]]
  Tn5Insertion <- rep(0, regionSize)
  Tn5Insertion[regionCounts$position] <- regionCounts$count
  
  # Get Tn5 bias 
  localRadius <- 50
  localAvg <- sapply(
    1:length(Tn5Insertion),
    function(i){
      mean(Tn5Insertion[max(1, i - localRadius):min(length(Tn5Insertion), i + localRadius)])
    }
  )
  obsBias <- Tn5Insertion / localAvg
  
  list(
    "insertion" = Tn5Insertion,
    "obsBias" = obsBias
  )
}