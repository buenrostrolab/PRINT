...
setMethod("getGroupATAC", 
          ...
          function(project,
            ...
            cat("Chunking data ..\n")
            groupIDs <- gtools::mixedsort(groups(project))
            if(length(groupIDs) == 0){
              stop("Need to set groups(project) slot first!")
            }
            ...
            for(i in 1:length(starts)){
              ...
              if(dim(chunkRegionCounts[[i]])[1] == 1){
                chunkRegionCounts[[i]] <- as.data.frame(t(chunkRegionCounts[[i]]))
              }
