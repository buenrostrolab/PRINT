...
getTFBindingSE <- function(project, # footprintingProject object
                           ...
                           nCores = 16, # Number of cores to use
                           regionList = NULL # Whether to read in data for only a subset of regions
){
  ...
  TFBSScores <- NULL
  regions <- regionRanges(project)
  chunks <- getChunkInterval(regions)
  ...
  TFBSRangesDf <- data.table::rbindlist(
    pbmcapply::pbmclapply(
      1:length(TFBSChunkFiles),
      function(chunkInd){
        
        # Get region indices for the current chunk
        chunkRegionInds <- chunks$starts[chunkInd]:chunks$ends[chunkInd]
        
        # Load a new chunk of data
        file <- TFBSChunkFiles[chunkInd]
        TFBSChunkData <- readRDS(paste0(TFBSDir, file))
        
        # Keep only selected regions
        if(!is.null(regionList)){
          TFBSChunkData <- TFBSChunkData[chunkRegionInds %in% regionList]
        }
        
        # Get TF binding site genomic ranges of the current chunk
        TFBSChunkRangesDf <- data.table::rbindlist(lapply(
          TFBSChunkData, 
          function(x){
            sites <- x$sites
            if(length(sites) > 0){
        ...
        # If a specific list of regions are specified, keep results in these regions
        if(!is.null(regionList)){
          TFBSChunkRangesDf <- TFBSChunkRangesDf[TFBSChunkRangesDf$regionInd %in% regionList, ]
        }
  ...
  TFBSScores <- data.table::rbindlist(
    pbmcapply::pbmclapply(
      ...
      function(chunkInd){
        ...
        # Get TF binding site genomic ranges of the current chunk
        TFBSChunkDf <- data.table::rbindlist(lapply(
          TFBSChunkData, 
          function(x){
            sites <- x$sites
            if(length(sites) > 0){
