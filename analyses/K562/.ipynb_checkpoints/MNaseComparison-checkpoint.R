...
# Load K562 MNase bigwig. The dataset ENCFF000VNN.bigWig is hg19
# https://www.encodeproject.org/files/ENCFF000VNN/
MNaseBigWig <- import.bw("../../data/K562/MNase/ENCFF000VNN.bigWig")
...
chunkDir <- "../../data/K562/chunkedFootprintResults/50/"
...
regionFilter <- which((regionDepth > 200) & (rowMaxs(MNaseTracks) > 2))
...
pdf(paste0("../../data/K562/plots/MNase/correlationHist.pdf"), width = 5, height = 5)
data.frame(corr) %>%
  ggplot(aes(x = corr)) +
...
positions <- -399:400
regionDepth <- rowSums(ATACTracks)
regionInd <- sample(regionFilter, 1)
...
pdf(paste0("../../data/K562/plots/MNase/region_", regionInd, ".pdf"), width = 5, height = 5)
p1 / p2 / p3 
dev.off()
