...
substructureFilter <- which(substructureSignal > 0.2)
...
###################################################################
# Compare substructure-gene correlation with CRE-gene correlation #
###################################################################
...
density <- get_density(plotX, plotY, n = 500)
...
vals <- c(plotData$CRESignedLogFDR, plotData$substructureSignedLogFDR)
vals <- vals[is.finite(vals)]
system("mkdir ../../data/BMMC/plots/RNACorr/")
png("../../data/BMMC/plots/RNACorr/RNACorr.png", width = 800, height = 700)
ggplot(plotData) +
  ...
  geom_hline(yintercept = -1, linetype ="dashed", 
             color = "black", size = 0.5) +
  xlab("cCRE-gene association signed -log10(FDR)") +
  ylab("sub-cCRE-gene association signed -log10(FDR)") +
...
####################################################
# Further characterize the sub-cCRE specific cases #
####################################################

selectedInds <- (abs(substructureSignedLogFDR) > 1) & (abs(CRESignedLogFDR) < 1)
numSelected <- sum(selectedInds)
selectedIndsCREs <- rowRanges(CRESE)[corCompare$CREInd[selectedInds]]
numProximal <- length(unique(findOverlaps(selectedIndsCREs, BuenRTools::hg38TSSRanges)@from))
numDistal <- numSelected - numProximal
